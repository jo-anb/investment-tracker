...
A Home Assistant integration that gathers brokerage balances, normalizes symbols, and exposes a structured portfolio model as sensors. The companion custom Lovelace card (`investment-tracker-card`) renders the same data with day-change totals, realized/unrealized splits, allocation pies, a plan tracker, and an in-card editor that lets you tweak your recurring investment plan using live asset metadata.
...
2. Restart Home Assistant so the integration icon and resources are registered.
...
The `investment-tracker-card` reads the sensors listed above and also fetches history for day change calculations. It offers:
	...
	- Asset list with price, quantity, profit/loss, and remap/history actions, plus a weekly-invested progress indicator that stacks buy activity by symbol inside the investment plan panel.
	- Charts (portfolio history, allocation pies, investment plan) along with a plan editor overlay that writes back to the integration via the new `investment_tracker.update_plan` service so configured totals and per-asset allocations persist with the entry options.
